...
public class DrawCircleTool extends JButton implements MouseListener, MouseMotionListener
{
    ...
    public DrawCircleTool(DrawingBoard _board)
    {
        super("Circle");
        board = _board;
    ...
    public void mouseDragged(MouseEvent e) 
    {
        ...
        if (x >= prevX && y >= prevY)
        {
            g2.drawOval(prevX, prevY, x - prevX, y - prevY);
        }
        else if (x <= prevX && y <= prevY)
        {
            g2.drawOval(x, y, prevX-x, prevY-y);
        }
        else if (x >= prevX && y <= prevY)
        {
            g2.drawOval(prevX, y, x-prevX, prevY-y);
        }
        else if(x <= prevX && y >= prevY)
        {
            g2.drawOval(x, prevY, prevX-x, y-prevY);
        }
